...
iomux_v3_cfg_t const pcie_pads[] = {
	MX6_PAD_GPIO_2__GPIO1_IO02 | MUX_PAD_CTRL(NO_PAD_CTRL),	/* RESET */
};
...
static void setup_display(void)
{
	...
	reg = IOMUXC_GPR2_BGREF_RRMODE_EXTERNAL_RES
		 ...
		 | IOMUXC_GPR2_LVDS_CH1_MODE_DISABLED//wendy
		 | IOMUXC_GPR2_LVDS_CH0_MODE_ENABLED_DI0;//wendy
	writel(reg, &iomux->gpr[2]);
 ...
 //Wendy-lvds power sequence timing//
iomux_v3_cfg_t const display_pads[] = {
	MX6_PAD_EIM_D31__GPIO3_IO31 | MUX_PAD_CTRL(NO_PAD_CTRL),  /* PANEL_DVDD_EN */
	MX6_PAD_KEY_COL2__GPIO4_IO10 | MUX_PAD_CTRL(NO_PAD_CTRL),  /* -VGA_EN */
	MX6_PAD_KEY_ROW2__GPIO4_IO11 | MUX_PAD_CTRL(NO_PAD_CTRL),  /* VGH_EN */
	MX6_PAD_KEY_ROW1__GPIO4_IO09 | MUX_PAD_CTRL(NO_PAD_CTRL),  /* +AVDD_EN- */
	MX6_PAD_DISP0_DAT8__GPIO4_IO29 | MUX_PAD_CTRL(NO_PAD_CTRL),  /* BLK_PWM */
	MX6_PAD_GPIO_0__GPIO1_IO00 | MUX_PAD_CTRL(NO_PAD_CTRL),  /* BLK_EN */
};

void set_iomux(void)
{
	imx_iomux_v3_setup_multiple_pads(display_pads, ARRAY_SIZE(display_pads));
}
EXPORT_SYMBOL_GPL(set_iomux);

void set_blk_en(void)
{
	printf("Wendy123: set_blk_en()\n");
	gpio_direction_output(IMX_GPIO_NR(1, 0), 1);
}
EXPORT_SYMBOL_GPL(set_blk_en);

void set_blk_pwm_en(void)
{
	printf("Wendy123: set_blk_pwm_en()\n");
	gpio_direction_output(IMX_GPIO_NR(4, 29), 1);
}
EXPORT_SYMBOL_GPL(set_blk_pwm_en);

void set_avdd_en(void)
{
	printf("Wendy123: set_avdd_en()\n");
	gpio_direction_output(IMX_GPIO_NR(4, 9), 1);
}
EXPORT_SYMBOL_GPL(set_avdd_en);

void set_vga_en(void)
{
        printf("Wendy123: set_vga_en()\n");
        gpio_direction_output(IMX_GPIO_NR(4, 10), 0);
}
EXPORT_SYMBOL_GPL(set_vga_en);

void set_vgh_en(void)
{
        printf("Wendy123: set_vga_en()\n");
        gpio_direction_output(IMX_GPIO_NR(4, 11), 1);
}
EXPORT_SYMBOL_GPL(set_vgh_en);

void set_panel_dvdd_en(void)
{
        printf("Wendy123: set_panel_dvdd_en()\n");
        gpio_direction_output(IMX_GPIO_NR(3, 31), 1);
}
EXPORT_SYMBOL_GPL(set_panel_dvdd_en);

void set_lvds_clk(void)/* LVDS signal */
{
	#if defined(CONFIG_VIDEO_IPUV3)
		setup_display();//LVDS CLK
	#endif
}
EXPORT_SYMBOL_GPL(set_lvds_clk);
...
/*
 * This section requires the differentiation between iMX6 boards, but
